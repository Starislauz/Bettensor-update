...
import psycopg2
from bettensor.miner.models.model_utils import SoccerPredictor, NFLPredictor, MLBPredictor
...
class PredictionsHandler:
    ...
    def process_model_predictions(
        ...
        if matched_games:
            ...
            for game_data, prediction in zip(matched_games, model_predictions):
                ...
                model_name = "NFL Model" if sport == "football" else "Soccer Model" if sport == "soccer" else "MLB Model"
                pred_dict = {
                    "prediction_id": str(uuid.uuid4()),
    ...
    def is_mlb_model_on(self):
        return self.models['baseball'].mlb_model_on if 'baseball' in self.models else False